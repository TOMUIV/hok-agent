...
class Kite(Skill):
    ...
    no_interrupt = True
    def update(self):
        """Move backward toward own base. Attack if enemy in range."""
        camp = self.ctx.camp()
        ...
        d = dist(self.ctx.px, self.ctx.py, self.ctx.ex, self.ctx.ey)
        ar = self.ctx.atk_range()
        # Move backward: target is behind hero (own base direction)
        base_x = -30000 if camp < 0 else 30000
        mx, my = direction_to(self.ctx.px, self.ctx.py, base_x, self.ctx.py)
        cfg = self.ctx.hero_config
